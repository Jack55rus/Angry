    ...
    def move_decision(self, current_board, step):
        ...
        for num, chip in enumerate(self.pawns): 
            if not chip.check_in_D(): # if this pawn hasn't reached the D-zone yet
                ...
                cond_1 = potential_content == 'D'
                if cond_1:
                    priors[num].append(1)
                cond_2 = (potential_content != self.char and potential_content != '*')
                if cond_2:
                    priors[num].append(2)
                cond_3 = current_coords is None
                if cond_3:
                    priors[num].append(3)
        ...
        if len(min_keys) != 0: # if the previous list is not empty (i.e. more than one pawn share min keys)
            ...
            previous_coords = self.pawns[chosen_pawn_ind].get_current_position()
            potential_returned_coords = self.pawns[chosen_pawn_ind].look_further(step)
            returned_index = chosen_pawn_ind
        else:
            previous_coords = self.pawns[min_keys[0]].get_current_position()
            potential_returned_coords = self.pawns[min_keys[0]].look_further(step)
